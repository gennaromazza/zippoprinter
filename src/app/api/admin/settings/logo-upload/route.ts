...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (file.size <= 0 || file.size > MAX_LOGO_BYTES) {
      return NextResponse.json(
        { error: "Il file logo supera il limite massimo di 4MB." },
        { status: 400 }
      );
    }
    ...
    const safeBaseName = (file.name || "logo")
      .toLowerCase()
      .replace(/\.[a-z0-9]+$/i, "")
      .replace(/[^a-z0-9_-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 40) || "logo";
    const storagePath = `logos/${photographer.id}/${Date.now()}-${safeBaseName}.${extension}`;
    ...
    const message = error instanceof Error ? error.message : "Errore upload logo.";
